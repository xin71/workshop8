...
export default class FeedItem extends React.Component {
  ...
  handleCommentPost(commentText) {
    postComment(this.state._id, getUserId(), commentText, (updatedFeedItem) => {
  ...
  handleLikeClick(clickEvent) {
    ...
    if (clickEvent.button === 0) {
      ...
      if (this.didUserLike()) {
        ...
        unlikeFeedItem(this.state._id, getUserId(), callbackFunction);
      } else {
        // User clicked 'like' button.
        likeFeedItem(this.state._id, getUserId(), callbackFunction);
      }
    }
  }
  ...
  didUserLike() {
    ...
    for (var i = 0; i < likeCounter.length; i++) {
      if (likeCounter[i]._id === getUserId()) {
        liked = true;
        break;
      }
    }
  ...
  onCommentLike(id, isLike) {
    ...
    if (isLike) {
      likeComment(this.state._id, id, getUserId(), cb);
    } else {
      unlikeComment(this.state._id, id, getUserId(), cb);
    }
  }
